#!/usr/bin/env bash
#SBATCH --job-name=mpnn_ENZ_REGIME
#SBATCH --output=/hpc/group/naderilab/darian/Enz/slurm/logs/mpnn_ENZ_REGIME_%j.out
#SBATCH --error=/hpc/group/naderilab/darian/Enz/slurm/logs/mpnn_ENZ_REGIME_%j.err
...
# Output FASTA files go to results/proteinmpnn/ENZ_REGIME/
# =============================================================================
...
for fa_file in sorted(glob.glob(f"{out_dir}/seqs/*.fa")):
    ...
    with open(fa_file) as f:
        ...
        for line in f:
            line = line.strip()
            if line.startswith(">"):
                seq_id = line[1:].split(",")[0].replace(">", "")
                seq_id = f"{design_id}_{seq_id}"
