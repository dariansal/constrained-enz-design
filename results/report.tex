...
Table~\ref{tab:results} summarizes median catalytic RMSD and pLDDT across all six experiments. For carbonic anhydrase, conditioning monotonically improves catalytic geometry; for elastase, conditioning monotonically worsens it — revealing a strikingly enzyme-class-dependent response (Figure~\ref{fig:rmsd}).

\begin{table}[h]
\centering
\caption{Summary of structural metrics per experiment. Catalytic RMSD (median [IQR]) is computed after Kabsch alignment of predicted catalytic triad C$\alpha$ atoms onto the reference crystal structure. Median pLDDT is the median across all predictions globally across the full designed sequence. Lower RMSD and higher pLDDT indicate better designs. Bold indicates best median value per enzyme per metric. $^\dagger$Fewer predictions due to ColabFold \texttt{--stop-at-score 70}: prediction halts after the first model exceeds pLDDT 70, so motif\_only sequences that fold well generate fewer than 5 model predictions per sequence.}
\label{tab:results}
\begin{tabular}{llrr r}
\toprule
Enzyme & Regime & Median Cat.\ RMSD (\AA{}) [IQR] & Median pLDDT & $N$ \\
\midrule
1PPF (Elastase) & \textbf{motif\_only} & \textbf{2.4 [4.5]} & \textbf{71.3} & 165$^\dagger$ \\
                & shell5              & 7.0 [3.5]          & 39.3          & 500 \\
                & shell8              & 9.1 [3.7]          & 30.5          & 500 \\
\midrule
1CA2 (CA-II)    & motif\_only         & 3.9 [6.8]          & \textbf{57.1} & 254$^\dagger$ \\
                & shell5              & 2.9 [3.8]          & 39.9          & 500 \\
                & \textbf{shell8}     & \textbf{2.4 [2.4]} & 29.8          & 500 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Distribution of catalytic C$\alpha$ RMSD per experiment}, computed after Kabsch alignment of the predicted catalytic triad onto the reference crystal structure. Diamond markers ($\Diamond$) indicate group means. For 1CA2 (right), median RMSD decreases monotonically from 3.9~\AA{} (motif\_only) to 2.9~\AA{} (shell5) to 2.4~\AA{} (shell8), with IQR narrowing from 6.8~\AA{} to 2.4~\AA{}. For 1PPF (left), median RMSD worsens monotonically: 2.4~\AA{} (motif\_only) to 7.0~\AA{} (shell5) to 9.1~\AA{} (shell8) — tighter conditioning drives elastase designs further from the native triad geometry. The broad distributions in motif\_only experiments reflect a mixture of designs that partially recapitulate the triad geometry and those that do not.}
\label{fig:rmsd}
\end{figure}

For 1CA2, shell conditioning produces a monotonic improvement: median RMSD drops from 3.9~\AA{} (motif\_only) to 2.9~\AA{} (shell5) and 2.4~\AA{} (shell8), a 38\% total reduction. IQR also drops from 6.8~\AA{} to 2.4~\AA{}, indicating that shell8 produces not just lower but far more consistent RMSD values. For 1PPF, the response is the opposite: median RMSD worsens monotonically from 2.4~\AA{} to 7.0~\AA{} to 9.1~\AA{}. Motif-only elastase designs are geometrically closest to the native triad, and adding contextual constraints only pushes the scaffold further away. IQR is relatively consistent across regimes for 1PPF ($\approx$3.5--3.7~\AA{} for shell5 and shell8), indicating that the misplacement is systematic rather than random.
...
\caption{\textbf{Structural variance per experiment}, defined as the mean pairwise catalytic RMSD among the 5 AlphaFold2 model predictions for each designed sequence (10 pairwise comparisons per sequence). Diamond markers ($\Diamond$) indicate group means. For 1CA2, median variance drops monotonically from 2.4~\AA{} (motif\_only) to 1.1~\AA{} (shell5) to 1.1~\AA{} (shell8) — a 54\% total reduction — indicating that all 5 AF2 models converge on nearly the same catalytic geometry for shell8 designs. For 1PPF, shell5 reduces median variance to 1.8~\AA{} but shell8 slightly increases it again to 2.1~\AA{}, suggesting the enforced geometry is not stable across all 5 models.}
\label{fig:variance}
\end{figure}

For motif\_only experiments, median variance is elevated for both enzymes (2.4~\AA{} for 1CA2, 2.8~\AA{} for 1PPF), indicating that without sufficient geometric context, the 5 AF2 models disagree on where the catalytic residues belong. Shell conditioning reduces this ambiguity substantially for 1CA2: shell8 brings median variance to 1.1~\AA{}, meaning all 5 models agree within $\sim$1~\AA{} on catalytic geometry, and this convergence corresponds to a correct geometry (median RMSD 2.4~\AA{}). For 1PPF, shell5 also reduces median variance (1.8~\AA{}) but shell8 increases it slightly (2.1~\AA{}), and neither shell regime converges on the native geometry — median RMSD climbs to 7.0 and 9.1~\AA{} respectively.
...
\caption{\textbf{Summary panel} showing mean catalytic RMSD, mean local pLDDT at catalytic residues, and mean structural variance per experiment. Error bars represent one standard deviation. Note: bars show means; see text for median values. For 1CA2, RMSD decreases monotonically and structural variance drops, while local pLDDT declines with tighter conditioning. For 1PPF, RMSD worsens monotonically — motif\_only yields the best geometry — while pLDDT and variance broadly follow the same trend as 1CA2. The divergent RMSD response between the two enzymes is the central finding of this study.}
\label{fig:summary}
\end{figure}
...
The central finding is that the response to motif shell conditioning is enzyme-class-dependent and directionally opposite between the two systems. For the Zn$^{2+}$-coordinated His triad in carbonic anhydrase, 8~\AA{} shell conditioning achieves a 38\% reduction in median catalytic RMSD (3.9~\AA{} $\to$ 2.4~\AA{}) and 54\% reduction in median structural variance (2.4~\AA{} $\to$ 1.1~\AA{}). For the Ser-His-Asp hydrogen-bond relay in elastase, tighter conditioning monotonically worsens median catalytic RMSD from 2.4~\AA{} (motif\_only) to 9.1~\AA{} (shell8) — the lowest RMSD is achieved with the least geometric constraint.
...
\bibitem{navia1989}
Navia, M.A., McKeever, B.M., Springer, J.P., et al.\ (1989). Structure of human neutrophil elastase in complex with a peptide chloromethyl ketone inhibitor at 1.84~\AA{} resolution. \textit{Proceedings of the National Academy of Sciences}, \textbf{86}, 7--11.

\bibitem{hakansson1992}
H\r{a}kansson, K., Carlsson, M., Svensson, L.A., Liljas, A.\ (1992). Structure of native and apo carbonic anhydrase II and structure of some of its adducts with inhibitors. \textit{Journal of Molecular Biology}, \textbf{227}, 1192--1204.
